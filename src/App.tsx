  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-[#050608]/85 backdrop-blur-xl border-b border-white/5">
      <div className="container mx-auto px-6 h-20 flex justify-between items-center">
        ...
        >
          <div className="relative h-10 w-40 flex items-center justify-start overflow-visible">
            <img 
              src="/logo.png" 
              alt="EsasWeb Logo" 
              className="h-full w-auto object-contain scale-[2.8] origin-left transition-transform duration-500 group-hover:scale-[3.1]"
            />
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          <button className="bg-white/5 border border-white/10 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-cyan-400 hover:text-black transition-all duration-500 flex items-center gap-3 group text-white">
...
export default function App() {
  return (
    <main className="min-h-screen bg-[#050608] text-white selection:bg-cyan-500/30 font-sans overflow-x-hidden pt-20">
      ...
      <section id="başlangiç" className="container mx-auto px-6 py-12 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto lg:h-[560px]">
          
          {/* Sol Büyük Kart - Dengeli Yükseklik Limiti */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="lg:col-span-7 bg-white/[0.02] border border-white/10 rounded-[2rem] p-8 flex flex-col justify-between h-[560px] relative overflow-hidden group"
          >
            ...
            <div className="relative z-10 space-y-4">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black uppercase leading-[0.95] tracking-tighter">
              ...
              <div className="flex gap-4 pt-2">
                <button className="bg-white text-black px-6 py-3.5 rounded-xl font-bold hover:bg-cyan-400 transition-all uppercase text-xs tracking-widest">Hizmetlerim</button>
                <button className="bg-white/5 border border-white/10 px-6 py-3.5 rounded-xl font-bold hover:bg-white/10 transition-all uppercase text-xs tracking-widest text-white font-mono">Hemen Başla</button>
              </div>
            </div>
            
            {/* Alt Kısım: Terminal Penceresi */}
            <div className="relative z-10 w-full bg-black/40 border border-white/5 rounded-xl font-mono text-[11px] text-slate-400 p-4 shadow-2xl backdrop-blur-md space-y-2 border-l-2 border-l-cyan-500/50">
              <div className="flex items-center justify-between border-b border-white/5 pb-1.5">
                ...
                <span className="text-[9px] text-slate-600 tracking-widest font-bold">ESASWEB_CORE_SECURITY.SH</span>
              </div>
          ...
          {/* Sağ Bento Kartları - Yükseklik Sınırlandırılmış Kolon */}
          <div className="lg:col-span-5 flex flex-col gap-6 h-auto lg:h-[560px]">
            
            {/* 1. KART: WEB TASARIM & GELİŞTİRME */}
            <div className="flex-1 bg-white/[0.02] border border-white/10 rounded-3xl p-6 relative overflow-hidden group hover:bg-white/[0.04] transition-all flex flex-col justify-between">
               <div>
                 <span className="label-mono text-[10px] text-slate-500 block mb-2 uppercase tracking-widest text-white/50">Hizmet Durumu</span>
                 <div className="flex items-center gap-3">
                   <div className="w-2.5 h-2.5 bg-cyan-400 rounded-full animate-pulse shadow-[0_0_10px_rgba(34,211,238,0.5)]" />
                   <h3 className="font-bold text-lg uppercase tracking-tight">Web Tasarım & Geliştirme</h3>
                 </div>
               </div>

               {/* Sabit h-32 yükseklik ile resmin tüm yapıyı bozması engellendi */}
               <div className="w-full h-32 rounded-xl overflow-hidden bg-white/5 border border-white/5 flex items-center justify-center p-1 my-2">
                 <img 
                   src="/web.png" 
                   alt="Web Tasarım" 
            ...
            <div className="flex-1 bg-white/[0.02] border border-white/10 rounded-3xl p-6 relative overflow-hidden group hover:bg-white/[0.04] transition-all flex flex-col justify-between">
              <div>
                <h3 className="font-bold text-lg uppercase mb-1 group-hover:text-cyan-400 transition-colors">Estetik Tasarım</h3>
                <p className="text-slate-400 text-xs leading-relaxed">Kullanıcı deneyimi ve görsel mükemmellik odaklı siber standartlarda arayüzler.</p>
              </div>

              {/* Sabit h-32 yükseklik */}
              <div className="w-full h-32 rounded-xl overflow-hidden bg-white/5 border border-white/5 flex items-center justify-center p-1 mt-2">
                <img 
      ...
      <footer className="py-12 text-center text-slate-700 text-[9px] font-mono uppercase tracking-[0.8em] opacity-40">
    ...
    <section id="hizmetler" className="container mx-auto px-6 py-20 border-t border-white/5">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
        <div className="lg:col-span-5">
          ...
          <h2 className="text-4xl md:text-6xl font-black uppercase leading-[0.9] tracking-tighter mb-8">YÜKSEK- <br />PERFORMANS <br /><span className="text-gradient">ÇÖZÜMLERİ</span>.</h2>
      ...
      <div className="bg-white/[0.02] border border-white/10 rounded-[3rem] p-10 md:p-20 relative overflow-hidden shadow-2xl shadow-cyan-950/10">
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-20">
          <div>
            ...
            <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-8 leading-none">SİSTEME <br /><span className="text-gradient">KATILIN</span>.</h2>
